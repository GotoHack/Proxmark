...
elements as the one in the original design, the Spartan 3 family is supported
...
it seems to do a good job when driven in LF mode at 125KSPS.
...
buzzer used is an inductive type (no piezzo) with a nominal 16 ohm resistance
...
TLV3502 part. Only one of these chips needs to be fitted to the board.
...
The antenna drivers, use two 74AC244, IC501 and IC502. These can operate from
a range of supply voltages. In this instance, depending on switch S1, they are
powered from either the 5v USB bus or the LiPo battery 3v to 4v range. The
design of the antenna drivers hasn't otherwise changed from the original.
...
back so the 2 clips on each side engage fully. Then the 4 pads of the metal
cover can be soldered to the board.

IC101 has what seems like a large via underneath it. This serves as a thermal
...
Also to maintain a low profile board, you may chose not to fit the SV301 JTAG
header. Instead adapt your JTAG programmer to end up in a 6 pin berg strip and
simply push it into the holes (noting the proper orientation) then hold it in
place at a slight angle to make proper contact for the brief period you need
to program the bootloader. After that it can be removed hopefully never to be
used again.
