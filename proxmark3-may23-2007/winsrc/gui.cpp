...
static void PaintGraph(HDC hdc)
{
    ...
    for(i = GraphStart; ; i++) {
        ...
        if(fabs(GraphBuffer[i]) > absYMax) {
            absYMax = (int)fabs(GraphBuffer[i]);
        }
...
void ShowGui(void)
{
    ...
    ScrollbackEdit = CreateWindowEx(WS_EX_CLIENTEDGE, WC_EDIT, "",
        WS_CHILD | WS_CLIPSIBLINGS | WS_VISIBLE | ES_MULTILINE |
        ES_AUTOVSCROLL | WS_VSCROLL, 0, 0, 0, 0, CommandWindow, NULL,
        NULL, NULL);

    CommandEdit = CreateWindowEx(WS_EX_CLIENTEDGE, WC_EDIT, "",
